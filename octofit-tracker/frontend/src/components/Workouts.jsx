...
const workoutsEndpoint = import.meta.env.VITE_CODESPACE_NAME
  ? (import.meta.env.DEV ? '/api/workouts/' : `https://${import.meta.env.VITE_CODESPACE_NAME}-8000.app.github.dev/api/workouts/`)
  : '/api/workouts/'

function Workouts() {
  const [workouts, setWorkouts] = useState([])
  const [error, setError] = useState('')
  useEffect(() => { fetchCollection('workouts', workoutsEndpoint).then(setWorkouts).catch((loadError) => setError(loadError.message)) }, [])
